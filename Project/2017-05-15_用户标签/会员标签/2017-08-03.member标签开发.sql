...
begin
  -- Call the procedure
  member_label_pkg.create_member_label(in_m_label_name      => 'ORDER_RECEIVE_CITY',
                                       in_m_label_desc      => '订购收货地址',
                                       in_m_label_type_id   => 1,
                                       in_m_label_father_id => null);
end;
/
...
消费特征指标  商品价位段偏好 （左开右闭）
    ...
    2000-3000元
    3000元以上
算法设计原则:通过对比用户近一年订过的商品进行判断，在哪个价位段的次数最多即为哪个标签，如有多个价位段出现的次数一样。取高价位段
*/
--*************************************************************************************************************

CREATE TABLE MEMBER_LABEL_429_STAGE AS
  SELECT F.MEMBER_KEY,
         H.M_LABEL_ID,
         H.M_LABEL_TYPE_ID,
         H.CREATE_USER_ID,
                    ...
                    FROM (SELECT C.MEMBER_KEY,
                            ...
                            FROM (SELECT TO_NUMBER(A.CUST_NO) MEMBER_KEY,
                                   ...
                                   WHERE A.ORDER_ID = B.ORDER_ID
                                     ...
                                     AND A.ADD_TIME >= TRUNC(SYSDATE) - 365) C
...
SELECT *
  FROM W_ETL_LOG A
 WHERE A.PROC_NAME = 'MEMBER_LABEL_PKG.MERGE_GOODS_PRICE_SEGMENT'
 ORDER BY A.START_TIME DESC;
...
INSERT INTO MEMBER_LABEL_HEAD
  (M_LABEL_ID,
   ...
   CURRENT_FLAG,
   SORT_FIELD)
  SELECT MEMBER_LABEL_HEAD_SEQ.NEXTVAL,
         B.VTEXT2 M_LABEL_NAME,
         B.VTEXT2 M_LABEL_DESC,
         1 M_LABEL_TYPE_ID,
         438 M_LABEL_FATHER_ID,
         SYSDATE CREATE_DATE,
         'yangjin' CREATE_USER_ID,
         SYSDATE LAST_UPDATE_DATE,
         'yangjin' LAST_UPDATE_USER_ID,
         1 CURRENT_FLAG,
         0 SORT_FIELD
    FROM (SELECT DISTINCT A.VTEXT2 FROM DIM_ZONE A) B;
                    ...
                    FROM (SELECT D.MEMBER_KEY,
                            ...
                            FROM (SELECT A.ORDER_ID,
                                   ...
                                   WHERE A.ORDER_ID = B.ORDER_ID
                                     ...
                                     AND B.ADD_TIME >= TRUNC(SYSDATE) - 365) D
...
SELECT *
  FROM W_ETL_LOG A
 WHERE A.PROC_NAME = 'MEMBER_LABEL_PKG.MERGE_ORDER_RECEIVE_CITY'
 ORDER BY A.START_TIME DESC;
...
SELECT * FROM MEMBER_LABEL_HEAD_LEVEL_V A WHERE A.LV1_ID IN (429, 438);

SELECT * FROM S_PARAMETERS2 FOR UPDATE;

SELECT MEMBER_LABEL_HEAD_SEQ.NEXTVAL FROM DUAL;
SELECT A.START_TIME,
       A.END_TIME,
       A.ETL_DURATION,
       A.ETL_RECORD_INS,
