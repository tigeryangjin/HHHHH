SELECT * FROM ODSHAPPIGO.OD_ORDER_ITEM A WHERE A.ORDER_NO = 20180605775559;
...
SELECT *
  FROM ( 
       --正向
       WITH ZHM_FORDER AS (SELECT A.ORDER_NO GUID_H,
                                  ...
                                  TO_CHAR(B.DELY_HOPE_DATE,
                                          'YYYYMMDDHH24MISS') FROM_TIME_H, --请求交货日期
                                  ...
                                  '' DCDEGREE, --礼券活动编号
                                  0 XYKCZ, --虚拟心意卡已自动充值
                                  TO_CHAR(B.LAST_PROC_DATE,
                                          'YYYYMMDDHH24MISS') TIMESTAMP_H, --时间戳
                                  TO_CHAR(B.LAST_PROC_DATE,
                                          'YYYYMMDDHH24MISS') TIMESTAMP_I, --时间戳
                                  '' QTY_UNIT, --销售单位
                                  '' CURRENCY, --货币
                                  ...
                                  '' Z400_FLAG, --400订单标记
                                  TO_CHAR(B.LAST_PROC_DATE,
                                          'YYYYMMDDHH24MISS') LAST_PROC_DATE,
                                  B.FREIGHT_AMT, --运费
                                  B.WH_CODE, --仓库
                                  CASE
                            ...
                            WHERE A.ORDER_NO = B.ORDER_NO
                              ...
                              AND B.ORDER_GB = 'HM20'
                              AND (B.ORDER_DATE >=
                                  TO_DATE(&IN_DATE_KEY, 'YYYYMMDD') - 2 AND
                                  B.ORDER_DATE <
                                  TO_DATE(&IN_DATE_KEY, 'YYYYMMDD') OR
                                  B.ORDER_DATE >=
                                  TO_DATE('20170101', 'YYYYMMDD') AND
                                  B.LAST_PROC_DATE >=
                                  TO_DATE(&IN_DATE_KEY, 'YYYYMMDD') - 2 AND
                                  B.LAST_PROC_DATE <
                                  TO_DATE(&IN_DATE_KEY, 'YYYYMMDD'))),
       --逆向
       ZHM_BORDER AS (SELECT A.ORDER_NO GUID_H, --对象 GUID
                       ...
                       WHERE A.ORDER_NO = B.ORDER_NO
                         ...
                         AND (B.ORDER_DATE >=
                             ...
                             TO_DATE(&IN_DATE_KEY, 'YYYYMMDD') - 2 AND
                             B.LAST_PROC_DATE <
                             TO_DATE(&IN_DATE_KEY, 'YYYYMMDD'))), ZECRD011 AS (SELECT GUID_H "0CRM_OHGUID",
                                                                                      GUID_I "0CRM_ITMGUI",
                                                                                      ...
                                                                                      ZEAMC013,
                                                                                      ZEAMC014
                                                                                 FROM ZHM_FORDER), ZECRD012 AS (SELECT GUID_H "0CRM_OHGUID",
                                                                                                                       GUID_I "0CRM_ITMGUI",
                                                                                                                       NULL "0RECORDMODE",
                                                                                                                       XDEL ZEAMC008,
                                                                                                                       PROCESS_TYPE "0CRM_PRCTYP",
       ...
       (SELECT DISTINCT "0BP_EMPLO",
         ...
         WHERE T.OBJVERS = 'A'
           ...
           AND P."0CRMPOSTDAT" BETWEEN T.DATEFROM AND T.DATETO),*/ ZTCRD001 AS (SELECT "0CRM_OBJ_ID" CRM_OBJ_ID,
         ...
         SELECT CRM_OBJ_ID,
                ...
                SYSDATE W_UPDATE_DT
           FROM ZTCRD001) Y
          WHERE Y.CRM_OHGUID = 20180605775559;
  ...
  FROM (SELECT A.ORDER_NO GUID_H,
         ...
         WHERE A.ORDER_NO = B.ORDER_NO
           ...
           AND (B.ORDER_DATE >= TO_DATE(&IN_DATE_KEY, 'YYYYMMDD') - 2 AND
               ...
               B.LAST_PROC_DATE < TO_DATE(&IN_DATE_KEY, 'YYYYMMDD'))) Y
 WHERE Y.GUID_H = 20180605775559;
