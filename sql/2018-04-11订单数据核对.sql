...
select * from dim_good a where a.item_code = 235804; --无资料
select * from dim_good a where a.item_code = 233724; --无资料
...
--***************************************************************************************
--1.
create table excel_product_tmp(过账日期 date,
                               订单渠道名称 varchar2(100),
                               商品编码 number,
                               有效订购金额 number,
                               有效订购数量 number);
...
--2.
select a.订单渠道名称, sum(a.有效订购金额) 有效订购金额
...
--3.
select a.过账日期, sum(a.有效订购金额) 有效订购金额
  from excel_product_tmp a
 where a.订单渠道名称 = '新媒体微信(2.0)'
 group by a.过账日期
 order by a.过账日期;
