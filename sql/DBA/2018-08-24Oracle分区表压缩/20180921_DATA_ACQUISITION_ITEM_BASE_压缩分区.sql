...
SELECT *
  FROM DBA_TABLES A
 WHERE A.OWNER = 'DW_USER'
   AND A.TABLE_NAME = 'DATA_ACQUISITION_ITEM_BASE';
...
--3.重建分区索引
ALTER INDEX DATA_ACQUISITION_ITEM_BASE_PK REBUILD;

SELECT 'ALTER INDEX ' || A.INDEX_NAME || ' REBUILD PARTITION ' ||
       A.PARTITION_NAME || ' NOLOGGING;' S
  FROM USER_IND_PARTITIONS A
 WHERE A.INDEX_NAME = 'DATA_ACQUISITION_ITEM_BASE_I1';
--4.
BEGIN
