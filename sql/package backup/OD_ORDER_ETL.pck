CREATE OR REPLACE PACKAGE OD_ORDER_ETL IS
  ...
  */
  ...
  PROCEDURE FIX_ODS_ORDER(IN_DATE_KEY IN NUMBER);
  /*
  功能名:       FIX_ODS_ORDER
  目的:         处理指定日期的数据修复
  作者:         yangjin
...
END OD_ORDER_ETL;
/
CREATE OR REPLACE PACKAGE BODY OD_ORDER_ETL IS
  ...
  BEGIN
    ...
    BEGIN
      DELETE ODSHAPPIGO.ODS_ORDER A WHERE A.CRMPOSTDAT = IN_DATE_KEY;
  ...
  PROCEDURE FIX_ODS_ORDER(IN_DATE_KEY IN NUMBER) IS
    /*
    功能说明：
    作者时间：yangjin  2018-04-09
    */
