/*
12/10-12/12未登录的白金及以上会员BP和会员等级，以及12/9-12/12未订购的白金及以上用户BP和会员等级
*/
SELECT A.MEMBER_BP, A.MEMBER_LEVEL
  FROM DIM_MEMBER A
 WHERE A.MEMBER_LEVEL IN ('HAPP_T4', 'HAPP_T5', 'HAPP_T6')
   AND NOT EXISTS
 (SELECT 1
          FROM (SELECT DISTINCT C.MEMBER_KEY
                  FROM FACT_PAGE_VIEW C
                 WHERE C.VISIT_DATE_KEY BETWEEN 20181209 AND 20181212
