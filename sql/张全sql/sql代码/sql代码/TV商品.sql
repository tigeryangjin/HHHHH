...
select ITEM_CODE from dim_good where  GROUP_ID=1000))  and order_state>10
...
select ITEM_CODE,sum(GOODS_NUM) GOODS_NUM,avg(GOODS_PAY_PRICE)  GOODS_PAY_PRICE,sum(GOODS_PAY_PRICE) GOODS_PAY_PRICE1 from factec_order_goods where  order_id in ( select order_id from factec_order where add_time between 20170101 and 20171022 and order_state>10  )
 and ITEM_CODE in(select ITEM_CODE from dim_good where  GROUP_ID=1000)
...
order_id in ( select order_id from factec_order where add_time between 20170101 and 20171022 and order_state>10  )
 and ITEM_CODE in(select ITEM_CODE from dim_good where  GROUP_ID=1000)
  group by ITEM_CODE) a
left join dim_ec_good b on a.ITEM_CODE=b.ITEM_CODE
)  where  GOODS_PAY_PRICE >0 group by GOODS_PAY_PRICE;
