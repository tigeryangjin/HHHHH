...
SELECT DISTINCT TO_NUMBER(A.MEMBER_KEY) MEMBER_KEY
 ...
 WHERE A.CREATE_DATE_KEY BETWEEN 20180801 AND 20180822
   AND A.MEMBER_KEY <> 0
   AND NOT EXISTS
 (SELECT 1
          FROM FACT_EC_ORDER_2 B
         WHERE A.MEMBER_KEY = B.CUST_NO
           AND TO_CHAR(B.ADD_TIME, 'YYYYMMDD') BETWEEN 20180801 AND 20180822)
UNION
SELECT DISTINCT TO_NUMBER(C.CUST_NO) MEMBER_KEY
  FROM FACT_EC_ORDER_2 C
 WHERE TO_CHAR(C.ADD_TIME, 'YYYYMMDD') BETWEEN 20180801 AND 20180822
