...
--1.2.W_ETL_LOG check
SELECT * FROM YJ_DAILY_CHECK_ERR_V T;

--odshappigo
SELECT * FROM YJ_DAILY_CHECK_ZMATERIAL_V;
SELECT * FROM YJ_DAILY_CHECK_ZBPARTNER_V;

--FACT_PAGE_VIEW    
--27
SELECT * FROM YJ_DAILY_CHECK_FPV_V;
--18
SELECT MAX(A.VISIT_DATE) FROM FACT_PAGE_VIEW@BITONEWBI A;
...
SELECT A.CREATEON
 ...
 WHERE EXISTS (SELECT 1
          FROM (SELECT MAX(C.ID) MAXID FROM ODSHAPPIGO.ODS_PAGEVIEW C) B
         WHERE A.ID = B.MAXID);
...
SELECT A.ITEM_CODE,
       ...
       B.ITEM_CODE,
       B.GOODS_COMMONID
  FROM DIM_GOOD A, DIM_EC_GOOD B
 WHERE A.ITEM_CODE = B.ITEM_CODE
   AND A.GOODS_COMMONID <> B.GOODS_COMMONID;
...
UPDATE DIM_GOOD A
   SET A.GOODS_COMMONID = NVL((SELECT B.GOODS_COMMONID
                               ...
                               WHERE A.ITEM_CODE = B.ITEM_CODE),
                              0)
 WHERE (A.GOODS_COMMONID IS NULL OR A.GOODS_COMMONID = 0)
   AND EXISTS
...
SELECT A.PROC_NAME,
       COUNT(1) CNT,
       ROUND(AVG(A.ETL_DURATION)) AVG_ETL_DURATION
  FROM W_ETL_LOG A
...
SELECT A.CRMPOSTDAT, COUNT(1)
 ...
 WHERE A.CRMPOSTDAT BETWEEN '20180701' AND '20180731'
 GROUP BY A.CRMPOSTDAT
 ORDER BY A.CRMPOSTDAT DESC;
