???create or replace procedure bihappigo.sp_sbi_upd_pageview is
  ...
  --898493 to 1039025
      ...
      update /*+rule*/ fact_page_view s
         set s.last_page_key =
          ...
          s.page_staytime = case
                                 when ceil((nvl((select /*+rule*/ min(h.visit_date)
                                                  from fact_page_view h
                                                 where h.id > var_array(i).id
                                                   and h.application_key=var_array(i).application_key
                                                   and h.visit_date<var_array(i).visit_date+600 / 86400
                                  ...
                                  ceil((nvl((select /*+rule*/  min(h.visit_date)
                                              from fact_page_view h
                                             where h.id > var_array(i).id
                                               and h.application_key=var_array(i).application_key
                                               and h.visit_date<var_array(i).visit_date+600 / 86400
    ...
    update /*+rule*/ fact_page_view z
     ...
     where --to_char(z.visit_date,'yyyy-mm-dd')='2015-06-06'
       z.id    between startpoint and endpoint;
