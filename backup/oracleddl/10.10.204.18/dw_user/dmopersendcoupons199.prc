???create or replace procedure dw_user.dmopersendcoupons199(postday in number) is
...
begin
  ...
  sp_name          := 'dmopersendcoupons199'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'oper_sendcoupons_3_199'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  --处理一月内3次199
  delete from oper_sendcoupons_3_199 where record_date_key = postday;
  insert into oper_sendcoupons_3_199
  ...
  merge into oper_sendcoupons_voucher m --fzq1表是需要更新的表
  ...
  --处理自然年内10次199
  delete from oper_sendcoupons_10_199 where record_date_key = postday;
  insert into oper_sendcoupons_10_199
    (member_key, last_date_key, order_total, record_date_key)
    (select member_key,
            posting_date_key as last_date_key,
            nums             as order_total,
