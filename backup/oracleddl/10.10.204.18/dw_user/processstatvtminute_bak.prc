???create or replace procedure dw_user.processstatvtminute_bak(startpoint in varchar2) is
  ...
  */
begin
  sp_name          := 'processstatvtminute'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    ...
    into i1
    from fact_page_view_vt;*/
  
    loop
      --exit when to_number(to_char(i, 'yyyymmddhh24mi')) >= to_number(i1);
      exit when to_number(to_char(i + 1 / 24 / 60, 'yyyymmddhh24mi')) >= to_number(startpoint);
      ...
      insert into stats_online_visitor_minute
        ...
        select w_statid_s.nextval,
               ...
               z3.uvcount,
               1
          from (select 1, nvl(count(distinct p3.vid), 2) as uvcount
      ...
      insert into stats_online_visitor_minute
        ...
        select w_statid_s.nextval,
               ...
               z4.uvcount,
               1
          from (select 1, nvl(count(distinct p3.vid), 0) as uvcount
