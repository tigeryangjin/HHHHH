???create or replace procedure dw_user.dmopersendcouponsorder(postday in number) is
...
begin
  ...
  sp_name          := 'dmopersendcouponsorder'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'oper_sendcoupons_order'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  insert into oper_sendcoupons_order
    (member_key, register_time)
    (select member_crmbp as member_key, min(member_time) as register_time
       from fact_ecmember@dw_datalink
      where member_crmbp != 0
        and member_crmbp not in (select member_key from oper_sendcoupons_order)
  ...
  begin
    select * bulk collect
      into var_array
      from (select member_key from oper_sendcoupons_order where member_key in (select member_key
              from fact_order@dw_datalink
               ...
               (select (case
                  ...
                  from (select mo.member_key,
                          ...
                          from (select m.member_key,
                                  ...
                                  from oper_sendcoupons_order m
                                  left join fact_member_register@dw_datalink mm
                                    on m.member_key = mm.member_key
                                 where m.member_key = var_array(i).member_key
                                 group by m.member_key) mo
                          left join fact_order@dw_datalink o
                            ...
                            select sum(case
                              ...
                              from (
                                     ...
                                     select ors.order_key,
                                       ...
                                       from oper_sendcoupons_order mo
                                       left join fact_order_reverse@dw_datalink ors
        ...
        --set oper_member_order
           set (first_order_time,
               ...
               (select 0 as first_order_time,
                  ...
                  from oper_sendcoupons_order mo
                  left join fact_member_register@dw_datalink mr
  ...
  begin
    select * bulk collect
      into var_arraya
      from (select member_key from oper_sendcoupons_order where state = 0);
