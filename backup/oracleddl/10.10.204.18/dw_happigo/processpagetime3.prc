???create or replace procedure dw_happigo.processpagetime3 is
  ...
  */
begin
  sp_name          := 'processpagetime3'; --需要手工填入所写PROCEDURE的名称
...
select  nvl(min(id),0) into startpoint from  fact_page_view where  visit_date_key=to_number(to_char((sysdate),'yyyymmdd'))
 and page_staytime_key=0;
  ...
  begin
    ...
    select * bulk collect
     ...
     where e.id between  startpoint and  startpoint+300000
     and e.visit_date_key=to_number(to_char((sysdate),'yyyymmdd'));
    ...
    update  fact_page_view z
     ...
     where z.id between startpoint and  startpoint+300000
     and z.visit_date_key=to_number(to_char((sysdate),'yyyymmdd'));
    commit;
  end;
