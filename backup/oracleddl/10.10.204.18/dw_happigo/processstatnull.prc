???create or replace procedure dw_happigo.processstatnull is
  ...
  */
begin
  sp_name          := 'processstatnull'; --需要手工填入所写PROCEDURE的名称
    ...
    select * bulk collect
     ...
     where e.frequency = 1
       and e.onlinevisitor=0
        and e.statdate=to_number(to_char(sysdate-1,'yyyymmdd'));
       
     
  
    for i in 1 .. var_array.count loop
    
   --insert into TMP_LI2 values
   --(var_array(i).START_TIME,var_array(i).END_TIME,var_array(i).APPLICATION_KEY);
  -- commit;
