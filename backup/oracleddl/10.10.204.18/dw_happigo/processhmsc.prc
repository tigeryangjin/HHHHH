???create or replace procedure dw_happigo.processhmsc is
  ...
  begin
    ...
    if v_count > 0 then
      insert into dim_hmsc
        (hmsc_key, hmsc, hmmd, hmpl, hmkw, hmci,insert_date)
      
        select w_hmsc_d_s.nextval, c.hmsc, c.hmmd, c.hmpl, c.hmkw, c.hmci,sysdate
