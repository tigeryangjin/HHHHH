???create or replace procedure dw_user.processpagetime is
...
begin
  ...
  select nvl(min(id), 0)
    into startpoint
    from fact_page_view
   where visit_date_key = to_number(to_char((sysdate - 1), 'yyyymmdd'))
  ...
  begin
    ...
    select *
     ...
     where e.id between startpoint and startpoint + 350000
    ...
    for i in 1 .. var_array.count loop
      --insert into test  (id ,time)  values ((var_array(i).id),sysdate) ;
      --commit;
      -- select /*+ index(a FPV_VISITOR) */ nvl(max(a.id), var_array(i).id)
      --  into mid
      ...
      -- select /*+ index(z FPV_ID) */z.page_key into p_key from fact_page_view z where z.id = mid;
    
      --   select /*+ index(z FPV_ID) */z.page_value into p_value from fact_page_view z where z.id = mid;
    
      select /*+ index(h FPV_VISITOR) */
       nvl(min(h.id), var_array(i).id)
        into minid
        from fact_page_view h
       where h.id > var_array(i).id
         and h.application_key = var_array(i).application_key
         and h.device_key = var_array(i).device_key;
    ...
    update fact_page_view z
     ...
     where z.id between startpoint and startpoint + 350000
