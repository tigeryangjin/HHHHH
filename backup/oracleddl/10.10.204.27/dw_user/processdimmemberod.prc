???create or replace procedure dw_user.processdimmemberod is
...
begin
  ...
  sp_name          := 'processdimmemberod'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    ...
    commit;
      select /*+ index(t1 MEMBER_ORDER_KEY) */ nvl(sum(case when t1.order_state = 1 then t1.order_amount end), 0),
             nvl(count(case when t1.order_state = 1 then t1.order_key end), 0),
             nvl(max( case when t1.order_state = 1 then t1.order_amount end), 0),
             nvl(min(case when t1.order_state = 1  then t1.order_date_key end), 0),
             nvl(max(case when t1.order_state = 1  then t1.order_date_key end), 0),
             nvl(count(case
                 ...
                 0),
             nvl(count(case
                         when t1.sales_source_key = 20  and t1.order_state = 1 and t1.sales_source_second_key not in
                              (20003，20013，20016，20018，20019，20023) then
                          1
                       end),
                 0),
             nvl(count(case
                         when t1.sales_source_key = 30 and t1.order_state = 1then
                          1
                       end),
                 0),
             nvl(sum(case
                       when t1.sales_source_key = 20 and t1.sales_source_second_key not in
             ...
             nvl(count(case
                         when t1.sales_source_key = 20 and
                              t1.sales_source_second_key in (20015, 20017) and t1.order_state = 1 then
                          1
                       end),
                 0),
             nvl(count(case
                         when t1.sales_source_key = 20 and
                              t1.sales_source_second_key in (20006, 20021) and t1.order_state = 1 then
                          1
                       end),
                 0),
             nvl(count(case
                         when t1.sales_source_key = 20 and
                              t1.sales_source_second_key in (20007, 20022) and t1.order_state = 1 then
                          1
                       end),
                 0),
             nvl(count(case
       ...
       where t1.member_key = var_array(i).member_bp
        and t1.order_date_key<=to_number(to_char(sysdate - 1, 'yyyymmdd'));
