???create or replace procedure dw_user.processupdateorder(postday in number) is
  ...
  crmpostdates number;
  /*total_nums   number;*/
  --根据发货前取消，以及有退换货的权责制订单，先删除这部分订单，在重新计算这些订单，发生日期不等于订购日期的才做计算
begin
  sp_name          := 'processupdateorder'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    ...
    select ZTCRMC04
      bulk collect
      into var_array_day
      from ((select ZTCRMC04
               from odshappigo.ods_order
              where CRM_PRCTYP in ('ZA08', 'ZB01', 'ZCR1')
                and ((zcr_on >= to_char(crmpostdates * 1000000) and
                    zcr_on < to_char((crmpostdates + 1) * 1000000) and
                    crmpostdat < to_char(crmpostdates)) or
                    crmpostdat = to_char(crmpostdates))
                and ZTCRMC01 < to_char(crmpostdates)
                and to_number(ZTCRMC04) > 0
              group by ZTCRMC04)
           
            union (select ZTCRMC04
    ...
    if var_array_day.count <= 0
    then
      s_etl.err_msg := '没有需要修改的数据';
      sp_sbi_w_etl_log(s_etl);
      return;
    end if;
  
    --先删除这部分的fact_order表数据
      ...
      select to_number(z.ZMATERIAL) as SALES_GOODS_ID, --goods sku
             ...
             (case
               when to_number(z.CRM_CRD_AT) > 0 then
                ...
                to_number(to_char(to_date(z.zgz_time,
                                          'yyyy-mm-dd hh24:mi:ss'),
                                  'hh24')) + 1
        ...
        from ( /*加上distinct去重 by yangjin 2017-07-25*/
              select distinct CREA_TIME,
                               CRM_CRD_AT,
                               CRM_ENDCST,
                               CRM_ITMTYP,
                               CRM_OBJ_ID,
                                      ...
                                      where CRM_PRCTYP in
                                        ...
                                        and ((zcr_on >=
                                            ...
                                            to_char((crmpostdates + 1) *
                                                      1000000) and
                                            crmpostdat <
                                            to_char(crmpostdates)) or
                                            crmpostdat =
                                            to_char(crmpostdates))
                                        and ZTCRMC01 < to_char(crmpostdates)
                                        and to_number(ZTCRMC04) > 0)
