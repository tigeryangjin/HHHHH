???create or replace procedure dw_user.processpvsession(startpoint in fact_page_view.id%type,
...
begin
  ...
  begin
    ...
    select *
      ...
      into var_array
      from fact_page_view
     where id between startpoint and endpoint -- 269548706 and 269685656 and session_key=0--startpoint and endpoint 
     order by id;
  
    --269548706  insert into TMP_LI3 values(1,'1');/*+ index(a SESSION_KEY_IDX) */ /*+ index(b VIDETIME_IDX) */269060200
  
    for i in 1 .. var_array.count loop
    
      select /*+ index(b VIDETIME_IDX) */
       nvl(max(b.session_key), 0) --count(b.session_key)
        into count_key
        from fact_session b
      ...
      then
        ...
        --insert into TMP_LI3 values(5,'5'); /*+ index(b SESSION_KEY_IDX) */  
        commit;
      
      else
        -- select  /*+ index(b VIDETIME_IDX) */ distinct b.session_key
        --    into s_key
        --    from fact_session b
        --  where b.vid = var_array(i).vid
        --     and b.end_time > var_array(i).visit_date - 1800 / 86400;
        update fact_page_view
           set session_key = count_key -- s_key
        
         where id = var_array(i).id;
      
        update /*+ index(a SESSION_KEY_IDX) */ fact_session a
           set a.end_time      = var_array(i).visit_date + 10 / 86400,
               a.end_date_key  = to_number(to_char((var_array(i)
                                                   .visit_date + 10 / 86400),
                                                   'yyyymmdd')),
               a.page_count    = page_count + 1,
               a.bonus_flag    = 1,
               a.left_page_key = var_array(i).page_key,
               a.member_key    = var_array(i).member_key
         where a.session_key = count_key; -- s_key;
        commit;
      end if;
