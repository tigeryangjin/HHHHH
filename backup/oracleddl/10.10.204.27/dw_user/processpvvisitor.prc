???create or replace procedure dw_user.processpvvisitor(s_startpoint in fact_session.id%type,
  ...
  begin
    ...
    select count(a.vid)
     ...
     where a.id between s_startpoint and s_endpoint
       and not exists
     (select 1 from FACT_VISITOR_REGISTER b where b.vid = a.vid);
  
    if v_count > 0
    then
      insert into FACT_VISITOR_REGISTER
        (id,
         VISTOR_KEY,
         vid,
         first_ip,
        ...
        select w_visitor_f_s.nextval,
          ...
          from (select p2.vid,
                       ...
                       p2.ver_name,
                       p2.agent,
                       get_ip_number(nvl(p2.fist_ip, '222.247.56.18')) as ip_int
                 ...
                 where p1.vid = p2.vid
                   ...
                   and not exists (select 1
                          from FACT_VISITOR_REGISTER b
                         where b.vid = p2.vid)) e;
