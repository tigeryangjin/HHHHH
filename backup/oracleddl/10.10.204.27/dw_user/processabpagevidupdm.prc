???create or replace procedure dw_user.processabpagevidupdm is --(startpoint number) is
...
begin
  ...
  sp_name          := 'processabpagevidupdm'; --需要手工填入所写PROCEDURE的名称
  ...
  declare
    ...
    type type_array is table of abpage_vidscore%rowtype index by binary_integer; --类似二维数组 
  ...
  begin
    ...
    select * bulk collect
      into var_array
      from abpage_vidscore t1
     where   t1.create_date_key between 20170101 and  20170508;
    ...
    for i in 1 .. var_array.count loop
     ...
     insert into tmp_0902 values (1, 2);
      commit;
    
      -- insert into abpage_vidscore
      --    (vid, vidscore)
      select nvl(max(z.vidscore),0)
        into varscore
