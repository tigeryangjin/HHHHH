???create or replace procedure dw_user.processopermorder2600(postday in number) is
...
begin
  ...
  sp_name          := 'processopermorder2600'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'oper_m_order_2600_3000'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  delete from oper_m_order_2600_3000 where record_date_key = postday;
  insert into oper_m_order_2600_3000
  ...
  using (select * from oper_m_order_2600_3000 where record_date_key = postday) mm -- 关联表
  on (m.member_key = mm.member_key) --关联条件
  when matched then --匹配关联条件，作更新处理
    update
       set m.d2600_ORDER_TIME      = mm.order_time,
           m.d2600_record_date_key = mm.record_date_key --此处只是说明可以同时更新多个字段。
    
  
  when not matched then --不匹配关联条件，作插入处理。如果只是作更新，下面的语句可以省略。
    insert
      (member_key, d2600_ORDER_TIME, d2600_record_date_key)
    values
