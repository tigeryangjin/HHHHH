???create or replace procedure dw_user.processzt_detail(startpoint in number) is
...
begin
  ...
  begin
        ...
        select var_array(i).page_name,
         ...
         (select *
                 ...
                 --to_number(var_array(i).page_value)
                   ...
                   and a2.goods_common_id =to_number(case
                       when regexp_like(a1.page_value, '.([a-z]+|[A-Z])') then
                        '0'
                       when regexp_like(a1.page_value, '[[:punct:]]+') then
                        '0'
    ...
    delete from fact_zt_ordergood;
    delete from fact_zt_session;
    commit;
  
    insert into fact_zt_ordergood
      (order_id,
       goods_num,
       goods_pay_price,
       vid,
    ...
    insert into fact_ztorders
      ...
      select t4.order_id,
             ...
             t4.brand_name,
             t4.is_tv
        from fact_zt_session t2, fact_zt_ordergood t4
