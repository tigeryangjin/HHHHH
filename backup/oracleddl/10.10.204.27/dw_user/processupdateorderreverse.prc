???create or replace procedure dw_user.processupdateorderreverse(postday in number) is
  ...
  crmpostdates number;
  --total_nums   number;
  --根据退货取消，先删除这部分订单，在重新计算这些逆向订单，发生日期不等于订购日期的才做计算
begin
  sp_name          := 'processupdateorderreverse'; --需要手工填入所写PROCEDURE的名称
      ...
      select to_number(z.ZMATERIAL) as SALES_GOODS_ID, --goods sku
             ...
             (case
               when to_number(z.ztcrmc02) = 0 then
                ...
                to_number(to_char(to_date(z.ztcrmc02,
        ...
        from (select distinct CRM_ENDCST,
               ...
               WHERE ((CRM_PRCTYP = 'ZB01'
                     ...
                     OR ZTCMC008 = '30' -- 用户回寄
                     OR ZTCMC008 = '40' --指定回寄快乐购
                     OR ZTCMC008 is null)) or crm_prctyp = 'ZA08')
                 AND crm_obj_id in
                     (select crm_obj_id
                        from odshappigo.ods_order_cancel
                       where CRM_PRCTYP = 'ZB01'
