???create or replace procedure dw_user.processopermemberordernums(postday in number) is
...
begin
  ...
  sp_name          := 'processopermemberordernums'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    select * bulk collect
      into var_arraya
      from (select member_key from oper_member_order where state = 0 and rownum<=postday);
...
exception
  when others then
    ...
    sp_sbi_w_etl_log(s_etl);
    return;
end processopermemberordernums;
/
