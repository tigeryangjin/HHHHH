???create or replace procedure dw_user.processpcpromotion(startpoint in number) is
  ...
  */
begin
  sp_name          := 'processpcpromotion'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'fact_daily_pcpromotion'; --此处需要手工录入该PROCEDURE操作的表格
    ...
    */
    update fact_pc_promotion a
       set (a.first_date, a.first_level2) =
           (select b.first_date, b.first_level2
              from (select rank() over(partition by c.vid order by c.maxdate, c.keyword, c.plan, c.adgroup, c.level1, c.level2) rn,
                           c.vid,
                           c.maxdate first_date,
                           c.level2 first_level2
                      from fact_pc_promotion c) b
             where a.vid = b.vid
               and b.rn = 1)
     where exists (select 1
              from (select rank() over(partition by e.vid order by e.maxdate, e.keyword, e.plan, e.adgroup, e.level1, e.level2) rn,
                           e.vid,
                           e.maxdate first_date,
                           e.level2 first_level2
                      from fact_pc_promotion e) d
             where a.vid = d.vid
               and d.rn = 1)
    ...
    insert into fact_daily_pcpromotion
      (active_date_key,
       vid,
       member_key,
       member_date,
