???create or replace procedure dw_user.processpvapplication(startpoint in fact_page_view.id%type,
  ...
  insert_rows number;
  update_rows number; --20181207 by yangjin
...
begin
  ...
  update_rows := sql%rowcount; --20181207 by yangjin
  commit;

  ---关联不上的维度，全部处理为-1
  update fact_page_view c
     set c.application_key = -1
   where c.application_key = 0
     and c.id between startpoint and endpoint;
  update_rows := update_rows + sql%rowcount; --20181207 by yangjin
  commit;

  /*日志记录模块*/
  s_etl.end_time       := sysdate;
  s_etl.etl_record_ins := insert_rows;
  s_etl.etl_record_upd := update_rows; --20181207 by yangjin
