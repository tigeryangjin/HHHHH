???create or replace procedure dw_user.processmemberpageordergoods(postday in number) is
...
begin
  sp_name          := 'processmemberpageordergoods'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'fact_member_ec_goods_sales_fq'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  begin
    delete from fact_member_ec_goods_sales_fq
     where member_key in
           (select member_key
              from fact_member_ec_goods_sales_ls
    ...
    insert into fact_member_ec_goods_sales_fq
      ...
      (select 0 as mat_status,
         ...
         from (select member_key,
                 ...
                 from fact_member_page_order_total
                where member_key in
                      (select member_key
                         from fact_member_ec_goods_sales_ls
                        where rn between postday + 1 and postday + 1000)
    ...
    insert into fact_member_ec_goods_sales_fq
      ...
      (select 1 mat_status,
         ...
         from (select t.item_code,
                      ...
                      tt.preference,
                      tt.grade
                 from fact_ec_goods_sales_mat_top4 t
                inner join (select c.*,
                                  (case
                                    when rownum < 5 then
                                     1
    ...
    if insert_rows > 0 then 
      ...
      insert into fact_member_ec_goods_sales_m (member_key,rn)(
      select max(member_key) member_key,max(rn) rn
                                             from fact_member_ec_goods_sales_ls
                                            where rn between postday + 1 and
                                                  postday + 1000);
    elsif postday!=0 and insert_rows=0 then
      delete from fact_member_ec_goods_sales_m;
    elsif insert_rows=0 and postday=0 then 
      delete from fact_member_ec_goods_sales_m;
      insert into fact_member_ec_goods_sales_m (member_key,rn)(select 0 member_key,0 rn from dual);
    end if;
...
exception
  when others then
    ...
    sp_sbi_w_etl_log(s_etl);
    return;
end processmemberpageordergoods;
/
