???create or replace procedure dw_user.processzt_detail2  (startpoint in number) is
  ...
  */
begin
  sp_name          := 'processztdetail2'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    delete from fact_ztgoods2;
    delete from fact_ztorders2;
    update dim_zhuangti_goods a2
       set a2.goods_common_id = get_commonid(a2.goods_common_id)
     where a2.zttype = 'WZT2';
        ...
        select var_array(i).page_name,
         ...
         where a1.session_key = var_array(i).session_key
           ...
           and exists (select *
                 ...
                 where a2.key = var_array(i).page_value
                   and a2.zttype = var_array(i).page_name
                   and a2.goods_common_id =
                       to_number(case
                                   when regexp_like(a1.page_value, '.([a-z]+|[A-Z])') then
                                    '0'
                                   when regexp_like(a1.page_value, '[[:punct:]]+') then
    ...
    delete from fact_zt_ordergood2;
    delete from fact_zt_session2;
    commit;
  
    insert into fact_zt_ordergood2
      (order_id,
       goods_num,
       goods_pay_price,
       vid,
      ...
      select t3.order_id,
       ...
       where t3.add_time = startpoint
         and exists (select *
                from fact_ztgoods2 p2
               where p2.goods_code = t3.goods_commonid);
    commit;
  
    insert into fact_zt_session2
      (session_key, start_time, end_time, member_key)
    
      select t1.session_key, t1.start_time, t1.end_time, t1.member_key
        from fact_session t1
       where t1.start_date_key = startpoint
         and exists (select *
                from fact_ztgoods2 p1
               where p1.session_key = t1.session_key);
  
    commit;
  
    insert into fact_ztorders2
      (order_id,
       goods_num,
       goods_pay_price,
       vid,
      ...
      select t4.order_id,
             ...
             t4.brand_name,
             t4.is_tv
        from fact_zt_session2 t2, fact_zt_ordergood2 t4
