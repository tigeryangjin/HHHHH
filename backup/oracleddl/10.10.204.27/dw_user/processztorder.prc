???create or replace procedure dw_user.processztorder (startpoint in number)
 is
      ...
      select max(id)
       ...
       (select *
               ...
               where p1.goods_code = var_array(i).goods_commonid
                 and p1.zt_id = to_number(case
                       when regexp_like(j0.page_value, '.([a-z]+|[A-Z])') then
                        '0'
                       when regexp_like(j0.page_value, '[[:punct:]]+') then
                        '0'
                       when j0.page_value is null then
                        '0'
                       when j0.page_value like '%null%' then
                        '0'
                       else
                        regexp_replace(j0.page_value, '\s', '')
                     end)
