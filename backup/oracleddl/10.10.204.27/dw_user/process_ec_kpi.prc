???create or replace procedure dw_user.process_ec_kpi(d_startpoint in varchar2,
                                           d_endpoint   in varchar2,
                                           w_startpoint in varchar2,
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval, x2.*
      from (select '01' as deno,
             ...
             where v1.scan_date_key >= to_number(d_startpoint)
               and v1.scan_date_key < to_number(d_endpoint)
 ...
 /*insert into kpi_ec_tbl
      ...
      from (select count(distinct(case
                                    when exists
                                     (select *
                                           ...
                                           where w1.scan_date_key = to_number(d_startpoint)
                                             and w1.vid = ccc.vid) then
                                     ccc.member_key
                                  end)) as odrs,
                   to_number(d_startpoint) as date_key
              from factec_order ccc
             where ccc.add_time = to_number(d_startpoint)
               and ccc.order_state > 10) xx,
           (select count(distinct w1.vid) as scantot,
                   to_number(d_startpoint) as date_key
              from dim_vid_scan w1
             where w1.scan_date_key = to_number(d_startpoint)) yy
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval,
      ...
      from (select trunc(count(distinct h3.vistor_key) / 0.95) as tot
             ...
             where h3.create_date_key = to_number(d_startpoint)
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval,
           ...
           '',
           sysdate
      from (select count(1) as tot
              from fact_ecmember h1
             where member_time = to_number(d_startpoint)
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval,
      ...
      from (select count(distinct h3.device_key) as tot
             ...
             where h3.visit_date_key = to_number(d_startpoint)
               and h3.application_key in (10, 20)) h5;
  commit;
  /* insert into kpi_ec_tbl
  select w_kpi_d_s.nextval,
...
insert into kpi_ec_tbl
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval, x3.*
      from (select '01' as deno,
                   ...
                   sysdate
              from dim_wechat h6
             where h6.subscribe_time = to_number(d_startpoint)) x3;
  commit;
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval, x3.*
      from (select '01' as deno,
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval,
           ...
           '',
           sysdate
      from (select to_number(d_startpoint) as datekey, count(1) as subtot
              from dim_wechat h7
             where h7.subscribe_time = to_number(d_startpoint)) a,
           
           (select to_number(d_startpoint) as datekey, count(1) as cantot
              from dim_wechat h7
             where h7.cancel_subscribe_time = to_number(d_startpoint)) b
  ...
  insert into kpi_ec_tbl
    select w_kpi_d_s.nextval,
      ...
      from (select (count(n2.open_id) - count(case
              ...
              from dim_wechat n2
             where n2.creat_time <= to_number(d_startpoint)) x6;
