???create or replace procedure dw_user.processmonthlytopic(startpoint in number,
...
begin
  ...
  sp_name          := 'processmonthlytopic'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'fact_monthly_topic'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  insert into fact_monthly_topic
    select to_date(to_char(sysdate, 'yyyymmdd'), 'yyyymmdd') as stats_date,
           startpoint || '至' || endpoint as tongji_range,
           ff.zt_name,
           ff.application_nm,
           sum(ff.totaluv) as monthlytotaluv,
           sum(ff.orderuv) as monthlyorderuv,
           sum(ff.ordercnt) as monthlyordercnt,
           sum(ff.goods_num) as monthlygoods_num,
           sum(ff.goods_amount) as monthlygoods_amount,
           trunc(sum(ff.orderuv) / sum(ff.totaluv), 3) as monthlyorderrate,
           avg(ff.pageavgtime) as monthlypageavgtime
  ...
  insert into fact_monthly_topic_goods
    select to_date(to_char(sysdate, 'yyyymmdd'), 'yyyymmdd') as stats_date,
           startpoint || '至' || endpoint as tongji_range,
           tt.zt_name,
           tt.item_code,
           ...
           tt.goods_price,
           tt.application_nm,
           sum(tt.goodpv) as monthgoodpv,
           sum(tt.gooduv) as monthgooduv,
           sum(tt.goodnum) as monthgoodnum,
           sum(tt.goodamount) as monthgoodamount
      from (select case
             ...
             where visit_date_key between startpoint and endpoint
               and page_name in ('KFZT', 'ZT_Detail2', 'WZT2', 'hdact')) tt
     where tt.zt_name is not null
     group by tt.zt_name,
              tt.item_code,
              tt.goods_name,
              tt.matdlt,
