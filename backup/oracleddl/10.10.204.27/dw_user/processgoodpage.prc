???create or replace procedure dw_user.processgoodpage(startpoint  in number,
  ...
  */
begin
  sp_name          := 'processgoodpage'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    ...
    select *
     ...
     where e.visit_date_key between startpoint and startpoint2 -- to_number(to_char(sysdate, 'yyyymmdd'))
    ...
    insert_rows := var_array.count; /*2018-09-12 by yangjin*/
  end;

  /*日志记录模块*/
  s_etl.end_time       := sysdate;
  s_etl.etl_record_ins := insert_rows;
  s_etl.err_sql        := '输入参数[startpoint]:' || startpoint ||
