???create or replace procedure dw_user.processmemberpageorder(postday in number) is
...
begin
  ...
  sp_name          := 'processmemberpageorder'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'fact_member_page_order'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  delete from fact_member_page_order where day_KEY = postday;

  insert into fact_member_page_order
    (item_code,
     member_key,
     pv_nums,
     cart_nums,
  ...
  if insert_rows > 0 then
    ...
    */
      ...
      begin
        ...
        for i in 1 .. var_array.count loop
          ...
          using (select var_array(i).member_key as member_key,
                        item_code,
                        sum(pv_nums) as pv_total /*浏览次数*/,
                        sum(cart_nums) as cart_total /*购物车次数*/,
                        sum(fav_nums) as fav_total /*收藏次数*/,
                        sum(order_nums) as order_total /*订购件数*/,
                        sum(cancel_nums) as cancel_total /*取消件数*/,
                        (sum(pv_nums) + sum(fav_nums) * 3 +
                        sum(cart_nums) * 5 -
                        (sum(order_nums) - sum(cancel_nums)) * 15) as USER_PREFERENCE /*用户偏好度*/,
                        (sum(pv_nums) + sum(fav_nums) * 3 +
                        sum(cart_nums) * 5 +
                        (sum(order_nums) - sum(cancel_nums)) * 15) as PL_PREFERENCE /*用户品类偏好度*/
                   from fact_member_page_order
    ...
    begin
      EXECUTE IMMEDIATE 'truncate table fact_member_ec_goods_sales_ls';
    end;
  
    --保存member_key,rn
    insert into fact_member_ec_goods_sales_ls
      (member_key, rn)
    ...
    --初始化rn
    delete from fact_member_ec_goods_sales_m;
    insert into fact_member_ec_goods_sales_m
      (member_key, rn)
      ...
      (select t.item_code, p.preference, p.grade, t.matxl
        ...
        inner join (select p.*,
                     ...
                     from (select g.matxl /*物料小类*/,
                                  sum(pl_preference) preference /*用户品类偏好度*/
                             from fact_member_page_order_total o
                             left join dim_ec_good g
                               on o.item_code = g.item_code
                            group by g.matxl
                            order by sum(pl_preference) desc) p
                    where rownum <= 15 /*用户品类偏好度前15名*/
                   ) p
           on t.matxl = p.matxl);
