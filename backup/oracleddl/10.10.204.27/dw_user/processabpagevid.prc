???create or replace procedure dw_user.processabpagevid is
...
begin
  ...
  sp_name          := 'processabpagevid'; --需要手工填入所写PROCEDURE的名称
  ...
  begin
    ...
    select * bulk collect
      into var_array
      from dim_vid_base e
     where e.create_date_key=to_number(to_char(sysdate - 1, 'yyyymmdd'));-- between 20170101 and 20170121;
  
    for i in 1 .. var_array.count loop
    
      insert into abpage_vidscore
        (vid, vidscore)
        select z.vid, z.vidscore
