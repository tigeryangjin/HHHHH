???create or replace procedure dw_user.processgoodresource(startpoint in number) is
...
begin
  ...
  sp_name          := 'processgoodresource'; --需要手工填入所写PROCEDURE的名称
  ...
  delete from tmp_0508_1;
  delete from tmp_0508_2;
  delete from tmp_0508_3;
  delete from tmp_0508_5;
 -- delete from fact_page_view_click;
  delete from tmp_0509_mindate;
  delete from  fact_daily_goodsresource_1 k where k.date_key=startpoint;
  commit;
 ...
 /* insert into fact_page_view_click
  ...
  insert into tmp_0508_3
    ...
    select m.item_code,
           ...
           sum(m.uv) as uv
      from tmp_0508_1 m
     group by m.item_code, m.item_name,m.page_resouce, m.page_cnnm;
  ...
  insert into fact_daily_goodsresource_1
    (date_key,
     item_code,
     item_name,
     page_resouce,
    ...
    select startpoint as date_key,
     ...
     where h1.item_code = h2.item_code(+)
       ...
       and h1.page_resouce=h2.page_resouce(+)
       and h1.page_cnnm = h2.page_cnnm(+);

  commit;

 /* update fact_daily_goodsresource_1 t1
     set t1.page_cnnm =
         (select t2.zt_name
