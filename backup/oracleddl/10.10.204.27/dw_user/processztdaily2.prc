???create or replace procedure dw_user.processztdaily2 (startpoint in number)
 is
...
begin
  ...
  sp_name          := 'processztdaily2'; --需要手工填入所写PROCEDURE的名称
  s_etl.table_name := 'fact_daily_zt2'; --此处需要手工录入该PROCEDURE操作的表格
  ...
  delete from fact_daily_zt2;
  --delete from fact_daily_zhuangti;
  commit;
  delete from fact_daily_zhuangti
   where to_char(stats_date, 'yyyy-mm-dd') =to_char(to_date(to_char(startpoint),'yyyy-mm-dd'),'yyyy-mm-dd')--   to_char(trunc(sysdate - 1),'yyyy-mm-dd')
   and key is not null
   ;
 -- commit;

  insert into fact_daily_zt2
    (zt_type,
     zt_id,
     zt_name,
     zt_start_date,
              ...
              from (select t0.*
                      from (select (select w1.zttypenm
                                     ...
                                     where w1.zttypenm = ll.page_name
                                       and w1.key = ll.page_value
                                       ) as zttypenm,
                                   (select w1.key as zt_id
                                      from dim_zhuangti w1
                                     where w1.zttypenm = ll.page_name
                                       and w1.key = ll.page_value ) as zt_id,
                                   (select w1.zt_name
                                      from dim_zhuangti w1
                                     where w1.zttypenm = ll.page_name
                                       and w1.key =ll.page_value ) as zt_name,
                                   (select w1.add_date_key
                                      from dim_zhuangti w1
                                     where w1.zttypenm = ll.page_name
                                       and w1.key =ll.page_value) as add_date_key,
                                   (select w1.end_date_key
                                      from dim_zhuangti w1
                                     where w1.zttypenm = ll.page_name
                                       and w1.key =ll.page_value ) as end_date_key,
                                   ll.page_value,
                             ...
                             where ll.visit_date_key =startpoint
                               and ll.page_key in (41928, 41930, 40410, 40411)
           ...
           (select x0.page_value,
                   (select w1.zttypenm
                     ...
                     where w1.zttypenm = x0.page_name
                       and w1.key =x0.page_value) as zttypenm,
                   (select w1.zt_name
                      from dim_zhuangti w1
                     where w1.zttypenm = x0.page_name
                       and w1.key =x0.page_value) as zt_name,
                   (select w1.add_date_key
                      from dim_zhuangti w1
                     where w1.zttypenm = x0.page_name
                       and w1.key = x0.page_value) as add_date_key,
                   (select o2.item_code
                      from dim_ec_good o2
            ...
            (select x1.zt_id,
               ...
               from (select distinct m1.zt_type,
                    ...
                    (select k0.zttype,
                      ...
                      where not exists
                      (select *
                               from fact_ztgoods2 k1
                              where k1.zt_id = k0.key
