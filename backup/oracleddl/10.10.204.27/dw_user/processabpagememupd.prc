???create or replace procedure dw_user.processabpagememupd(startpoint number) is
...
begin
  ...
  sp_name          := 'processabpagememupd'; --需要手工填入所写PROCEDURE的名称
    ...
    for i in 1 .. var_array.count loop
      ...
      select z.memscore
        ...
        from (select x.member_key, sum(vidtot + memtot) as memscore
                from (select var_array(i).member_key as member_key,
                       ...
                       where member_bp = var_array(i).member_key) x,
                     (select var_array(i).member_key as member_key,
                             nvl(sum(v0.score), 0) as vidtot
                        from dim_vid_base v0
                       where exists
                       (select *
                                from dim_vid_mem v1
                               where v1.member_key = var_array(i).member_key
                                 and v1.vid = v0.vid)) y
